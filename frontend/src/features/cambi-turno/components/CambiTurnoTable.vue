...
import type { CambiTurnoTableEmits, CambiTurnoTableProps } from '@/features/cambi-turno/types'

const props = defineProps<CambiTurnoTableProps>()

const emit = defineEmits<CambiTurnoTableEmits>()

function canAnswerAsColleague(richiesta: CambiTurnoTableProps['richieste'][number]) {
  return (
...
<template>
  <EiraTable :empty="richieste.length === 0" empty-message="Nessuna richiesta di cambio turno.">
    <table>
      <thead>
        ...
        <tr v-for="richiesta in richieste" :key="richiesta.id">
          ...
          <td class="actions">
            <template v-if="canAnswerAsColleague(richiesta)">
              <Button label="Accetta" size="small" @click="emit('colleagueResponse', richiesta, true)" />
              <Button label="Rifiuta" size="small" severity="secondary" @click="emit('colleagueResponse', richiesta, false)" />
            </template>
