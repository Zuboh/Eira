...
import type { AssegnazioneTurno, ConsegnaSbarForm, Paziente, SbarFormErrors } from '@/features/sbar/types'
...
function submit() {
  errors.value = validateSbarForm(form.value, { checkPazienteTurno: !props.isEditing })
...
<template>
  <Dialog v-model:visible="visible" :header="isEditing ? 'Modifica consegna' : 'Nuova consegna'" modal :style="dialogStyle.lg">
    <form class="form" novalidate @submit.prevent="submit">
      <FormField v-if="showPaziente" label="Paziente" required :error="errors.paziente_id">
        <Select v-model="form.paziente_id" :options="pazienti" optionLabel="cognome" optionValue="id" placeholder="Seleziona paziente" />
      </FormField>
      <FormField v-if="showTurno" label="Turno" required :error="errors.turno_id">
        <Select v-model="form.turno_id" :options="assegnazioni" optionLabel="turno_id" optionValue="turno_id" placeholder="Seleziona turno" />
      </FormField>
